...
vconn_connect(struct vconn *vconn)
{
    ...
    do {
        ...
        switch (vconn->state) {
        case VCS_CONNECTING:
            //printf("HNH VCS_CONNECTING %d\r\n",VCS_CONNECTING);
            vcs_connecting(vconn);
            break;

        case VCS_SEND_HELLO:
            //printf("HNH VCS_SEND_HELLO %d\r\n",VCS_SEND_HELLO);
            vcs_send_hello(vconn);
            break;

        case VCS_RECV_HELLO:
            //printf("HNH VCS_RECV_HELLO %d\r\n",VCS_RECV_HELLO);
            vcs_recv_hello(vconn);
            break;

        case VCS_CONNECTED:
            //printf("HNH VCS_CONNECTED %d\r\n",VCS_CONNECTED);
            return 0;

        case VCS_SEND_ERROR:
            //printf("HNH VCS_SEND_ERROR %d\r\n",VCS_SEND_ERROR);
            vcs_send_error(vconn);
            break;

        case VCS_DISCONNECTED:
            //printf("HNH VCS_DISCONNECTED %d\r\n",VCS_DISCONNECTED);
            return vconn->error;

        default:
            //printf("HNH vconn->state %d\r\n",vconn->state);
            NOT_REACHED();
        }
...
{
    //printf("HNH do_recv in vconn.c\r\n");
    int retval = (vconn->class->recv)(vconn, msgp);
    if (!retval) {
        struct ofp_header *oh;
        //printf("HNH (vconn->class->recv)(vconn, msgp); OK\r\n");
        COVERAGE_INC(vconn_received);
        if (VLOG_IS_DBG_ENABLED()) {
            char *s = ofp_to_string((*msgp)->data, (*msgp)->size, 1);
            VLOG_DBG_RL(&ofmsg_rl, "HNH %s: received: %s", vconn->name, s);
            free(s);
        }
